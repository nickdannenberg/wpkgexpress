...
class ProfilesController extends AppController {
	...
	function index() {
		if ($this->RequestHandler->isXml())
			$this->set('profiles', $this->Profile->getAllForXML());
		else {
	...
	function view($id = null) {
		...
		$this->set('profile', $this->Profile->get($id, true));
	}
	...
	function _canDisable($id) {
		...
		if (is_array($hosts) && count($hosts) > 0) {
			//$this->set('records', $hosts);
			//$this->set('field', 'name');
			return "The Profile '" . $this->Profile->field('id_text') . "' cannot be disabled because it is the main profile for the following host(s): " . $this->element('RecordInUseByLinks', array('records' => $hosts, 'field' => 'name'));
		} else
			return true;
	}
